            ...
            <form action="{{ route('surgeries.update', $surgery) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="line_id" id="line_id" class="form-select @error('line_id') is-invalid @enderror" required>
                            ...
                            @foreach($lines as $line)
                                <option value="{{ $line->id }}" {{ (old('line_id', $surgery->line_id) == $line->id) ? 'selected' : '' }}>
                                    {{ $line->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $surgery->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea name="notes" id="notes" rows="2" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $surgery->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                                ...
                                @foreach($equipment as $item)
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="equipment_ids[]"
                                            ...
                                            {{ in_array($item->id, old('equipment_ids', $surgery->equipment->pluck('id')->toArray())) ? 'checked' : '' }}>
                                ...
                                @foreach($instrumentistas as $staff)
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="staff_ids[]"
                                            ...
                                            {{ in_array($staff->id, old('staff_ids', $surgery->staff->pluck('id')->toArray())) ? 'checked' : '' }}>
