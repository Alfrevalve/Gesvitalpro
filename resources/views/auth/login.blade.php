<x-guest-layout>
    <div class="flex justify-between items-center mb-8">
        ...
        <div>
            ...
            <a href="{{ route('register') }}" class="ml-2 text-[#0097CD] font-medium hover:text-[#0F3061]">SIGN UP</a>
        </div>
    </div>

    <div class="mb-8">
        <h2 class="text-2xl font-bold text-[#0F3061]">Welcome Back!</h2>
        <p class="text-gray-500 mt-1">Sign in to your account</p>
    </div>

    @if (session('status'))
        <div class="mb-4 text-sm font-medium text-[#65D7CA]">
            {{ session('status') }}
        </div>
    @endif
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <input type="checkbox" id="remember_me" name="remember"
                       class="h-4 w-4 text-[#0097CD] focus:ring-[#65D7CA] border-gray-300 rounded">
                <label for="remember_me" class="ml-2 text-sm text-gray-600">Remember me</label>
            </div>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-[#0097CD] hover:text-[#0F3061]">
                    Forgot password?
                </a>
            @endif
        </div>
